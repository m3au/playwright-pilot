...
test.describe('JSONPlaceholder Users Load Test', () => {
  test('should handle normal load', async () => {
    const configPath = 'tests/performance/challenges/jsonplaceholder/config/users-load.yml';
    ...
    if (result.results) {
      ...
      const thresholds = getJSONPlaceholderThresholds('users');
